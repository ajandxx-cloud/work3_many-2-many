...
alternative. The full choice set for request $r$ is $\mathcal{B}_r \cup \{0\}$.

The MNL choice probability that passenger $r$ (of type $k$) selects bundle $b$ is:
...
maximizes expected system objective contribution, then presents it to the passenger. The
passenger accepts with probability $P_{rb^*}^k$ and rejects with probability $P_{r0}^k$.
The realized acceptance $z_r$ is therefore a Bernoulli random variable with parameter
$P_{rb^*}^k$.
...
(Equation~\ref{eq:outside-prob}). The realized acceptance indicator is:
