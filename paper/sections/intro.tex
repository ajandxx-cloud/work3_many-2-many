...
$M_r^D$ for dropoff), (b)~explicit Multinomial Logit (MNL) passenger choice with
...
Passenger response is modelled via a Multinomial Logit (MNL) model with three
...
$\mathcal{B}_r = M_r^P \times M_r^D$ and computes MNL choice probabilities;
(2)~passenger response, which determines the accepted bundle based on MNL utilities;
...
\begin{enumerate}
  \item First formulation of many-to-many DRT with bidirectional meeting point sets
        ($M_r^P$, $M_r^D$) and MNL passenger choice as a coupled three-layer model.
  \item MILP exact benchmark and rolling horizon ALNS heuristic for the bidirectional
...
coupled model and the MNL choice structure.
Section~\ref{sec:algorithm} describes the solution methodology: the MILP exact
benchmark and the rolling horizon ALNS heuristic.
