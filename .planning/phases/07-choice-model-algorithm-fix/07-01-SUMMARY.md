...
- Not caused by the `choice_probability` to `accept_probability` refactor
...
| Threat ID | Status |
|-----------|--------|
| T-07-01 — accept_probability return value in (0,1) | Mitigated: denominator is always exp_outside + exp_bundle >= 1.0 > 0; numerator exp_bundle > 0; result always in (0,1) |
...
None. The binary logit formula is fully wired: `accept_probability` calls `mnl_utility` with real beta coefficients from `PassengerType`.
