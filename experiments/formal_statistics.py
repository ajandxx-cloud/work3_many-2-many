...
DEFAULT_MANUSCRIPT_TABLE = Path("manuscript/tables/phase06_main_behavioral_table.tex")
DEFAULT_MANUSCRIPT_FIGURE = Path("manuscript/figures/fig04_phase06_main_efficiency_coverage.png")
...
def _ensure_failure_ledger(results_dir: Path, ledger_path: Path = DEFAULT_LEDGER) -> Path:
    """Rebuild the durable failure ledger from formal raw rows when absent."""
    from experiments.phase06_coverage_controls import append_control_failures_to_ledger

    raw_path = results_dir / "coverage_controls/matched_coverage/raw_results.csv"
    append_control_failures_to_ledger(raw_path, ledger_path)
    return ledger_path
...
def write_manuscript_main_behavioral_table(
    table_dir: Path = DEFAULT_TABLE_DIR,
    output_path: Path = DEFAULT_MANUSCRIPT_TABLE,
) -> Path:
    """Write the manuscript-facing main behavioral table from formal CSVs."""
    main_table = pd.read_csv(table_dir / "main_behavioral_table.csv")
    ci_table = pd.read_csv(table_dir / "paired_bootstrap_ci.csv")
    all_rows = main_table[main_table["scale"].astype(str) == "all"].copy()
    method_order = ["FullModel", "DoorToDoor", "SingleSidedPickup", "SingleSidedDropoff"]
    all_rows["method_order"] = all_rows["method"].apply(
        lambda method: method_order.index(method) if method in method_order else len(method_order)
    )
    all_rows = all_rows.sort_values("method_order")
    ...
    metric_labels = {
        "served_share": "Served share",
        "choice_rejection_rate": "Choice rejection",
        "feasibility_rejection_rate": "Feasibility rejection",
        "vkm_per_served_trip": "Vehicle-km per served trip",
        "vkm_per_original_request": "Vehicle-km per original request",
    }
    door_ci = ci_table[
        (ci_table["scale"].astype(str) == "all")
        & (ci_table["baseline"] == "DoorToDoor")
        & (ci_table["metric"].isin(comparison_metrics))
    ].copy()
    door_ci["metric_order"] = door_ci["metric"].apply(comparison_metrics.index)
    door_ci = door_ci.sort_values("metric_order")
    paired_count = int(door_ci["n_valid_pairs"].max()) if not door_ci.empty else 0
    ...
    for _, row in all_rows.iterrows():
        lines.append(
            f"{row['method']} & "
            f"{_fmt_pct(row['served_share_mean'])} & "
            f"{_fmt_pct(row['choice_rejection_rate_mean'])} & "
            f"{_fmt_pct(row['feasibility_rejection_rate_mean'])} & "
            f"{_fmt_num(row['vkm_per_served_trip_mean'])} & "
            f"{_fmt_num(row['vkm_per_original_request_mean'])} \\\\"
        )
    lines.extend(
        [
            "\\midrule",
    ...
    for _, row in door_ci.iterrows():
        metric = row["metric"]
        if metric in {"served_share", "choice_rejection_rate", "feasibility_rejection_rate"}:
            mean = f"{_fmt_pp(row['mean_difference'])} pp"
            interval = f"[{_fmt_pp(row['ci_low'])}, {_fmt_pp(row['ci_high'])}] pp"
        else:
            mean = _fmt_num(row["mean_difference"])
            interval = f"[{_fmt_num(row['ci_low'])}, {_fmt_num(row['ci_high'])}]"
        lines.append(
            f"{metric_labels[metric]} & \\multicolumn{{2}}{{r}}{{{mean}}} & "
            f"\\multicolumn{{2}}{{r}}{{{interval}}} & {int(row['n_valid_pairs'])} \\\\"
        )
    lines.extend(
        [
            "\\bottomrule",
            ...
            "are descriptive formal primary behavioral evidence, not paired hypothesis-test significance claims.",
            "\\end{minipage}",
            "\\end{table}",
            "",
        ]
...
def verify_phase06(results_dir: Path, phase_dir: Path) -> dict:
    ...
    ledger_path = _ensure_failure_ledger(results_dir, DEFAULT_LEDGER)
...
def write_manuscript_main_behavioral_figure(
    plot_dir: Path = DEFAULT_PLOT_DIR,
    output_path: Path = DEFAULT_MANUSCRIPT_FIGURE,
) -> Path:
    """Copy the formal main Phase 6 plot into the manuscript figure directory."""
    source_path = plot_dir / "phase06_main_efficiency_coverage.png"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(source_path, output_path)

    metadata_path = plot_dir / "plot_metadata.json"
    metadata = _read_json(metadata_path) if metadata_path.exists() else {}
    metadata["manuscript_copies"] = [
        {
            "source": str(source_path),
            "target": str(output_path),
            "evidence_role": "formal Phase 6 primary behavioral evidence",
        }
    ]
...
def run_closeout(
    ...
    phase_dir.mkdir(parents=True, exist_ok=True)
    _ensure_failure_ledger(results_dir, DEFAULT_LEDGER)
    ...
    manuscript_table = write_manuscript_main_behavioral_table(table_dir)
    plots = write_plots(table_dir, plot_dir)
    manuscript_figure = write_manuscript_main_behavioral_figure(plot_dir)
    ...
    return {
        ...
        "tables": [str(path) for path in [*tables, manuscript_table]],
        "plots": [str(path) for path in [*plots, manuscript_figure]],
